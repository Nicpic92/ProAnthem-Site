    ...
    try {
        ...
        if (event.httpMethod === 'POST') {
            ...
            if (setlistId && resourceType === 'songs') {
                ...
                try {
                    // Check ownership of both setlist and song
                ...
                // --- FIX: Insert uses the user's band_id ---
        ...
        if (event.httpMethod === 'PUT') {
            const body = JSON.parse(event.body);
            if (setlistId && resourceType === 'songs') {
                 // The check for band_id already happens here
        ...
        if (event.httpMethod === 'DELETE') {
             // --- FIX: Both delete operations must check for band_id ---
            if (setlistId && !resourceType) {
                 await client.query('BEGIN');
