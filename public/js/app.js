...
function ProAnthemApp(isDemo = false) {
    ...
    this.el = {
        ...
        cloneSetlistBtn: document.getElementById('clone-setlist-btn'),
...
ProAnthemApp.prototype.attachEventListeners = function() {
    ...
    this.el.cloneSetlistBtn?.addEventListener('click', this.handleCloneSetlist.bind(this));
...
ProAnthemApp.prototype.initializeDemoSong = function() {
    ...
    this.el.durationInput.value = this.songData.duration;
...
ProAnthemApp.prototype.loadSong = async function(id) {
    ...
    try {
        ...
        this.songData = { id: data.id, title: data.title || '', artist: data.artist || '', duration: data.duration, audio_url: data.audio_url, song_blocks: Array.isArray(data.song_blocks) ? data.song_blocks : [], tuning: data.tuning ?? 'E_STANDARD', capo: data.capo ?? 0, transpose: data.transpose ?? 0 };
        this.el.titleInput.value = this.songData.title;
        this.el.artistInput.value = this.songData.artist;
        this.el.durationInput.value = this.songData.duration || '';
...
ProAnthemApp.prototype.handleImport = function() {
    ...
    if (result.blocks.length > 0) {
        this.songData = { id: null, title: 'Imported Song', artist: '', duration: '', audio_url: null, song_blocks: result.blocks, tuning: result.metadata.tuning, capo: result.metadata.capo, transpose: 0 };
        this.el.titleInput.value = this.songData.title;
        this.el.artistInput.value = this.songData.artist;
        this.el.durationInput.value = '';
        if (this.el.songSelector) this.el.songSelector.value = 'new';
...
ProAnthemApp.prototype.parseLineForRender = function(rawLine) { return UI.parseLineForRender(rawLine); };
ProAnthemApp.prototype.populateTuningSelector = function() { UI.populateTuningSelector(this.el.tuningSelector, this.CONSTANTS.TUNINGS); };
ProAnthemApp.prototype.loadSheetList = async function(selectId = null) { await UI.loadSheetList(this.el.songSelector, api); if(selectId) this.el.songSelector.value = selectId; };
...
ProAnthemApp.prototype.handleAddSetlistNote = function() { const noteText = this.el.newSetlistNoteInput.value.trim(); const noteDuration = this.el.newSetlistNoteDuration.value.trim(); if (!noteText) return; const noteId = `note_${Date.now()}`; const li = document.createElement('li'); li.className = 'flex justify-between items-center p-2 bg-gray-750 border border-dashed border-gray-600 rounded cursor-grab'; li.dataset.itemId = noteId; li.dataset.itemType = 'note'; li.dataset.duration = noteDuration || '0:00'; const gripHandle = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-grip-vertical inline-block mr-2 text-gray-400" viewBox="0 0 16 16"><path d="M7 2a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm3 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0zM7 5a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm3 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0zM7 8a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm3 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm-3 3a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm3 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm-3 3a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm3 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"/></svg>`; li.innerHTML = `<div class="flex-grow"><span>${gripHandle}<em class="text-indigo-300">${noteText}</em></span></div><div class="flex items-center gap-4"><span class="font-mono text-sm">${noteDuration || '0:00'}</span><button data-action="remove-item" class="btn btn-danger btn-sm">Remove</button></div>`; this.el.songsInSetlist.appendChild(li); this.el.newSetlistNoteInput.value = ''; this.el.newSetlistNoteDuration.value = ''; this.saveSetlistOrderAndNotes(); };
...
ProAnthemApp.prototype.parseTimeToSeconds = function(timeStr) { if (!timeStr || typeof timeStr !== 'string') return 0; const parts = timeStr.split(':').map(Number); if (parts.length === 2) { return (parts[0] * 60) + (parts[1] || 0); } if (parts.length === 1) { return parts[0]; } return 0; };
ProAnthemApp.prototype.formatSecondsToTime = function(seconds) { const h = Math.floor(seconds / 3600); const m = Math.floor((seconds % 3600) / 60); const s = Math.round(seconds % 60); let timeStr = `${m.toString().padStart(2, '0')}:${s.toString().padStart(2, '0')}`; if (h > 0) { timeStr = `${h}:${timeStr}`; } return timeStr; };
ProAnthemApp.prototype.recalculateSetlistTime = function() { const items = document.querySelectorAll('#songsInSetlist li'); let totalSeconds = 0; items.forEach(item => { totalSeconds += this.parseTimeToSeconds(item.dataset.duration); }); this.el.setlistTotalTime.textContent = `Total Time: ${this.formatSecondsToTime(totalSeconds)}`; };
ProAnthemApp.prototype.getSongKeyInfo = function(song) { let parts = []; const firstChordMatch = song.song_blocks?.[0]?.content?.match(/\[([^\]]+)\]/); if (firstChordMatch) { const originalKey = firstChordMatch[1]; const soundingKey = this.transposeChord(originalKey, (song.transpose || 0) + (song.capo || 0)); parts.push(`Key: ${soundingKey}`); } if (song.capo > 0) { parts.push(`Capo ${song.capo}`); } if (song.tuning && song.tuning !== 'E_STANDARD') { const tuningName = this.CONSTANTS.TUNINGS[song.tuning]?.name || song.tuning; parts.push(tuningName); } return parts.join(' | '); };
ProAnthemApp.prototype.handleStartShow = function() { const setlistId = this.el.setlistSelector.value; if (setlistId) { window.open(`/show.html?id=${setlistId}`, '_blank'); } else { alert('Please select a setlist to start the show.'); } };
