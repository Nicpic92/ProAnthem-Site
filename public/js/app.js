...
ProAnthemApp.prototype.renderPreview = function() { const renderTransposedTab = (block) => Fretboard.renderTransposedTab(block, this.songData.tuning, this.songData.capo, this.songData.transpose, this.CONSTANTS.TUNINGS, this.CONSTANTS.FRETBOARD_CONFIG); UI.renderPreview(this.el.livePreview, this.songData.song_blocks, renderTransposedTab); };
ProAnthemApp.prototype.getFretFromClick = function(evt, svgEl) { const blockId = svgEl.id.split('-').pop(); const block = this.songData.song_blocks.find(b => b.id === blockId); return Fretboard.getFretFromClick(evt, svgEl, block.strings || 6, this.CONSTANTS.STRING_CONFIG, this.CONSTANTS.FRETBOARD_CONFIG); };
...
ProAnthemApp.prototype.loadSheetList = async function(selectId = null) { await UI.loadSheetList(this.el.songSelector, this.api); if(selectId) this.el.songSelector.value = selectId; };
